...
export class PortfolioSyncService {
    ...
    async syncGitHub(userId: string, username: string) {
        this.logger.log(`Syncing GitHub repositories for user ${userId}, username: ${username}`);
        ...
        try {
            ...
            this.logger.log(`Synced ${portfolioItems.length} GitHub repositories for user ${userId}`);
            return { success: true, count: portfolioItems.length };
        } catch (error) {
            this.logger.error(`Failed to sync GitHub for user ${userId}: ${error.message}`);
            throw new Error(`GitHub sync failed: ${error.message}`);
        }
    }

    async syncBehance(userId: string, username: string) {
        this.logger.log(`Syncing Behance projects for user ${userId}, username: ${username}`);
        ...
        try {
            const response = await firstValueFrom(
                this.httpService.get(`https://api.behance.net/v2/users/${username}/projects`, {
                    params: {
                        api_key: apiKey,
                        per_page: 10,
                    },
            ...
            this.logger.log(`Synced ${portfolioItems.length} Behance projects for user ${userId}`);
            return { success: true, count: portfolioItems.length };
        } catch (error) {
            this.logger.error(`Failed to sync Behance for user ${userId}: ${error.message}`);
            throw new Error(`Behance sync failed: ${error.message}`);
        }
    }

    async syncDribbble(userId: string, username: string) {
        this.logger.log(`Syncing Dribbble shots for user ${userId}, username: ${username}`);
        ...
        if (!accessToken) {
            ...
            return { success: false, message: 'Dribbble access token not configured' };
        }
        ...
        try {
            ...
            this.logger.log(`Synced ${portfolioItems.length} Dribbble shots for user ${userId}`);
            return { success: true, count: portfolioItems.length };
        } catch (error) {
            this.logger.error(`Failed to sync Dribbble for user ${userId}: ${error.message}`);
    ...
    async syncAllPlatforms(userId: string) {
        ...
        if (user.dribbbleUsername) {
            try {
                results.dribbble = await this.syncDribbble(userId, user.dribbbleUsername);
            } catch (error) {
