...
import ClientReputation from '@/components/ClientReputation';
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-sm">
                    ...
                    >
                        ...
                        <div className="flex flex-col lg:flex-row divide-y lg:divide-y-0 lg:divide-x divide-slate-800">
                                    ...
                                    {success ? (
                                        ...
                                        <>
                                            <div className="space-y-2">
                                                <label className="text-sm font-medium text-slate-300">Submit As Profile</label>
                                                <div className="flex flex-wrap gap-2">
                                                    <button
                                                        type="button"
                                                        onClick={() => setSelectedProfileId(null)}
                                                        className={`px-4 py-2 rounded-xl border text-xs font-bold transition-all ${selectedProfileId === null
                                                    ...
                                                    {specializedProfiles.map((profile) => (
                                                        <button
                                                            ...
                                                            className={`px-4 py-2 rounded-xl border text-xs font-bold transition-all ${selectedProfileId === profile.id
                                                                ? 'bg-blue-600/10 border-blue-500/50 text-blue-400'
                                                                : 'bg-slate-950 border-slate-800 text-slate-500 hover:border-slate-700'
                                                                }`}
                                                        >
                                            ...
                                            {portfolioItems.length > 0 && (
                                                <div className="space-y-3">
                                                    ...
                                                    <div className="grid grid-cols-2 gap-2">
                                                        {portfolioItems.filter(item => selectedProfileId ? item.specializedProfileId === selectedProfileId : true).map((item) => (
                                                            <button
                                                                key={item.id}
                                                                type="button"
                                                                onClick={() => {
                            ...
                            <div className="w-full lg:w-72 p-6 bg-slate-900/50 space-y-6">
                                {clientId && <ClientReputation clientId={clientId} />}

                                <div className="p-4 rounded-xl bg-amber-500/10 border border-amber-500/20">
                                    <div className="flex items-center gap-2 text-amber-500 font-bold text-xs mb-2">
                                        <Sparkles className="w-3.5 h-3.5" />
                                        Safety Tip
                                    </div>
                                    <p className="text-[10px] text-amber-500/80 leading-relaxed">
                                        Never share your personal contact information before a contract is started on the platform. Keep payments protected through our escrow system.
                                    </p>
                                </div>
                            </div>
                        </div>
